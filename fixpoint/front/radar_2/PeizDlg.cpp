...
BOOL CPeizDlg::OnInitDialog() 
{
	...
	m_port1=9008;
    UpdateData(FALSE); 
	CString strip="10.8.5.124";
	CString strip1="10.8.5.123";
...
recv:
	...
	while(1)
	{
	  redata:
		...
		{
			if(m_savState1==0||m_savState2==0)
			{
				m_nCurID=0;
				m_storeNum=m_savNum;
				m_savState1=1;
		...
		*/
		...
		{    
			//开始数据处理
			length=Alg->GetResult(lidar1Data,lidar2Data);		//处理并保存数据

			DataNum.Format("%f %f",scanPosInfo[0],scanPosInfo[1],scanPosInfo[2],scanPosInfo[3]);
		...
		if(rettag4==true)
		{
			...
			if (rettag)
			{
				...
				for ( i = 0;i<pData->num;i++)
				{
					...
					ytControllor.ScanStorage(storeID);
				}
			}
		}
	}
