...
spaceDownSamplingThreshold = str2num(paravalue.alg.spacedownsampling);
...
elseif strcmp(method, 'cube3')
  ...
  elseif(nVarargs==1)
    ...
    while(flag)
      ...
      if(length(dataOut)>=spaceDownSamplingThreshold)
        cubeLen = cubeLen+10;
        flag = 1;
      else
        flag = 0;
...
elseif strcmp(method, 'skipPoint')
  if(nVarargs>1)
    ...
    elseif(nVarargs==0)
      skipPointNumber = ceil(length(dataIn)/spaceDownSamplingThreshold); % 假设最多保留65000个顶点
    end
