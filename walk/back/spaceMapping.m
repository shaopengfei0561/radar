function [ dataOut, varargout ] = spaceMapping( dataIn, method, varargin  )
% 基于点云数据将单体目标进行空间位置的映射
%   Detailed explanation goes here
%   yuzhijun@wsn.cn, 2015/5/14
% 例子：
%   dataOut = spaceMapping( dataIn, 'cube', 1, 1500  );
...
if strcmp(method, 'cube')
  ...
  %% 方法1：空间立方体降采样，取落在立方体里点的平均值或单纯取立方体的某个顶点(采用Hu的方法提升效率)
elseif strcmp(method, 'cube3')
  ...
  elseif(nVarargs==4)
    ...
    while(flag)
      tempData = dataIn; % - ones(length(dataIn),1)*borderDown;
      tempData1 = floor(tempData(:,3)/cubeLen1);
