function [ dataOut, varargout ] = spaceMapping2( dataIn, method, varargin  )
% 基于点云数据将单体目标进行空间位置的映射，基于spaceMapping改写--------------------------仅用于直接通过点云数据计算体积！！！！！！！！！！
...
elseif strcmp(method, 'cube3')
  ...
  elseif(nVarargs==2)
    ...
    tempData1 = ones(length(tempData),1); % --------------------------仅用于直接通过点云数据计算体积！！！！！！！！！！
    tempData2 = floor(tempData(:,1:2)/cubeLen);
  ...
  elseif(nVarargs==4)
    ...
    while(flag)
      tempData = dataIn; % - ones(length(dataIn),1)*borderDown;
      tempData1 = ones(length(tempData),1); % --------------------------仅用于去除地面数据！！！！！！！！！！
  ...
  elseif(nVarargs==5)
    ...
    borderDown = min(dataIn);
    subMethod = varargin{1};  % 降采样方式，1为取顶点，2为取立方体的中心
    cubeLen1 = varargin{2}; % 高度上的尺度Z
    cubeLen2 = varargin{3}; % 平面上X方向的尺度
    cubeLen3 = varargin{4}; % 平面上Y方向的尺度
    nTargets = varargin{5}; % 标准货物的个数
    % cubeLen2 = 1300; % 初始值
    flag = 1;
    while(flag)
      tempData = dataIn; % - ones(length(dataIn),1)*borderDown;
      tempData1 = ones(length(tempData),1); % --------------------------仅用于直接通过点云数据计算体积！！！！！！！！！！
      tempData2 = floor(tempData(:,1)/cubeLen2);
      tempData3 = floor(tempData(:,2)/cubeLen3);
      tempData = [tempData2,tempData3,tempData1];
      [dataOut, tempOut1, tempOut2] = unique(tempData,'rows');
      varargout{1} = cubeLen1;
      varargout{2} = cubeLen2;
      dataOut = [dataOut(:,1:2)*cubeLen2,dataOut(:,3)*cubeLen1]; % + ones(length(dataOut),1)*borderDown;
